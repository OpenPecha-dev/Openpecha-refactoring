...
def get_repo_names(headers):
    ...
    for page_num in range(1,175):
        response = requests.get(f"https://api.github.com/orgs/Openpecha-Data/repos?page={page_num}&per_page=100", headers=headers)
        data = response.json()
        for info in data:
...
if __name__ == "__main__":
    ...
    Path(f"./final_repo_names.txt").write_text(repo_names, encoding='utf-8')
